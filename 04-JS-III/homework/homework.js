...
function agregarNumeros(numeros) {
  ...
  //var SumaArray= numeros.reduce(function(acc,elem){
    //return acc + elem
  //});
  //return SumaArray;
  var sumador = 0
...
function numeroMasGrande(numeros) {
  ...
  // Tu código:
  //numeros = [20, 200, 100, 5]
 var indicebase = numeros[0];
 for (var i = 0; i < numeros.length; i++) {
   if(numeros[i] > indicebase){
     return numeros[i]
   } 
 }
}
...
function breakStatement(numero) {
  ...
  for (var i = 0; i < 10; i++) {
    numero = numero + 2;    
          //no va numero[i] porque es un argumento "numero" y no un array (dice numero RECIBIDO)
    if(numero === i){
      break  
